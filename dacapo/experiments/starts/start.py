...
def match_heads(model, weights, old_head, new_head ):
    ...
    for label in new_head:
        if label in old_head:
            ...
            for key in ["prediction_head.weight","prediction_head.bias","chain.1.weight","chain.1.bias"]:
    ...
    def initialize_weights(self, model):
        ...
        try:
            if self.old_head and self.new_head:
                ...
                logger.warning(f"matched heads from run {self.run}, criterion: {self.criterion}")
