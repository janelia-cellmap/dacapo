...
class Train(GenericTrain):
    ...
    Args:
        ...
        heads (subclass of ``torch.nn.Module``):

            The heads to train.

        losses:

            The torch losses to use per head.
        ...
        loss_inputs (``List(dict)``, ``string`` or ``int`` -> :class:`ArrayKey`):

            List of dictionaries with one dictionary per head.
    ...
    def start(self):
        ...
        for i in range(len(self.losses)):
            if isinstance(self.losses[i], torch.nn.Module):
                self.losses[i] = self.losses[i].to(self.device)
        ...
        if checkpoint is not None:
            ...
            for i, head in enumerate(self.heads):
                self.heads[i][1].load_state_dict(checkpoint[f"head_{i}"])
    ...
    def train_step(self, batch, request):
        ...
        if isinstance(model_outputs, torch.Tensor):
            ...
            raise RuntimeError(
                "Torch train node only supports return types of ",
                f"torch.Tensor from model.forward(). not {type(model_outputs)}",
            )
        ...
        if batch.iteration % self.save_every == 0:
            ...
            for i, head in enumerate(self.heads):
                state_dicts[f"head_{i}"] = head.state_dict()
    ...
    def __collect_provided_loss_inputs(self, batch, loss_inputs):
        ...
        return self.__collect_provided_arrays(
            loss_inputs, batch, expect_missing_arrays=True
        )
